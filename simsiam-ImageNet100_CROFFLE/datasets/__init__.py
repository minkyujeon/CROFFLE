...
def get_dataset(dataset, data_dir, transform, memory='False', train=True, download=True, debug_subset_size=None):
    if dataset == 'mnist':
    ...
    #     dataset = torchvision.datasets.ImageNet(data_dir, split='train' if train == True else 'val', transform=transform, download=download)
        ...
        dataset = torchvision.datasets.ImageFolder('/home/minkyu/data/imagenet100/val', transform=transform)      
