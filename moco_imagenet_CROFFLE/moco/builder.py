...
class ShuffleConv(nn.Module):
    def __init__(self, feature_dim=512, temperature=10):
        super(ShuffleConv, self).__init__()
        self.c3_projector = projection_conv(feature_dim,512,512)
        self.c3_predictor = prediction_conv(512,128,512)
        self.temp = temperature
        self.ce= nn.CrossEntropyLoss()

    def forward(self, c3, c3_prime):
...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k):
        ...
        c3, q = self.encoder_q(im_q)  # queries: NxC
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            c3_prime, k = self.encoder_k(im_k)  # keys: NxC
        ...
        return logits, labels, c3, c3_prime
